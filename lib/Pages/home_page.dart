...
import 'package:bitslaundry/Authentication/login.dart';
import 'package:provider/provider.dart';
import 'package:bitslaundry/Model/cartModel.dart';
import 'package:bitslaundry/Components/cartItemBottomSheet.dart';
import 'order_details.dart';
import 'package:bitslaundry/Pages/orderedPage.dart';
import 'dart:ui';
import 'Address/select_address.dart';
import 'package:bitslaundry/Pages/profile.dart';
import 'package:shared_preferences/shared_preferences.dart';
...
@override
  Future getUserData() async{
    ...
    await FirebaseFirestore.instance.collection("bitlaun").doc(phone);
    documentReference.get().then((datasnapshot) {
       ...
       if (datasnapshot.exists) {
        ...
        setState(() {
          name = datasnapshot['providerProfile']['name'];
          fSPhone = datasnapshot['providerProfile']['phone'];
          address =  datasnapshot['address'][0];
  ...
  Future getCardData() async{
    DocumentReference documentReference =
    await FirebaseFirestore.instance.collection("appData").doc('card_data');
    documentReference.get().then((datasnapshot) {
      ...
      if(datasnapshot=null){
        ...
        ;
        setState(() {
          cardData = datasnapshot['cardData'];
  ...
  Future getCarosoleData() async{
    DocumentReference documentReference =
    await FirebaseFirestore.instance.collection("appData").doc('image_carousel');
    documentReference.get().then((datasnapshot) {
      if(datasnapshot=null){
        return Text("Waiting");
      }
    ...
    return
      ...
      Scaffold(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        child: SafeArea(child: Row(
                          ...
                          children: <Widget>[
                            Icon(Icons.account_box_rounded,size : 70,color :Colors.white),
                            ...
                            Column(
                              ...
                              snapshot != null ?
                              Text(name,style: TextStyle(color: Colors.white,fontSize: 10),):Text(""),
                                ...
                                Row(
                                  children: <Widget>[
                                    //
                                    snapshot !=null ? 
                                    Text(phone,style: TextStyle(color:Colors.white,fontSize: ScreenUtil().setSp(40)),):Text(""),
                                  ],
                                ),
                              ],
                            )
        ...
        appBar: AppBar(
          ...
          title: Text("BIT LAUNDRY "),
          brightness: Brightness.dark,
          actions: <Widget>[
            IconButton(icon: Icon(Icons.help_outline,color: Colors.white,size: 25,), onPressed: () {  },)
          ],
